...
class ViewController: UIViewController,secondprotocol,thirdprotocol {
    ...
    let  filemanager:CustomFileManager? = nil
    ...
    func savenotes() {
        UserDefaults.standard.set(self.noteslist, forKey: "notes")
        UserDefaults.standard.synchronize()
        
//        for i in 0...self.noteslist.count{
//            print("sheinaxa")
//            filemanager?.createFile(name: "notes.txt", text: self.noteslist[i])
//        }
}
    func readnotes(){
        if let notes=UserDefaults.standard.array(forKey: "notes") as? [String]{
            self.noteslist=notes
        }
//        print("waikita")
//
//        var c=filemanager?.readText(name: "notes.txt")
//        print(c)
//
    }
//
